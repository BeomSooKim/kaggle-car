...
parser.add_argument('-ld','--load_dir', required = False, default = './models/', type = str, help = 'directory for saving model')
...
def test(args):
    ...
    test_data = Dataloader(test_list, 'D:/dataset/kaggle_car/test', test_trn)
    ...
    # load best model
    nfold = len(os.listdir(os.path.sep.join(['models',load_dir])))
    pred_ensemble = []
    for i in range(nfold):
        model_list = glob(os.path.sep.join(['models', model_dir, 'fold{}'.format(i+1), '*.pt']))
        model_selected = sorted(model_list, key = lambda x: float(x.split('_')[-1].rstrip('.pt')))[-1]
        print('best model for fold{} : {}'.format(i+1, model_selected))
        ...
        for n in range(tta):
            ...
            with torch.no_grad():
                for idx, img in enumerate(test_loader):
                    ...
                    print('{}/{}'.format(idx+1, len(test_list) / 128), end = '\r')
    ...
    result.to_csv(os.path.sep.join(['models',load_dir,'submission.csv']), index = False)
